...
void draw_boxes(cv::Mat mat_img, std::vector<bbox_t_3d> result_vec, std::vector<std::string> obj_names) {
	...
	int left  = i.bbox.x;
	int right = i.bbox.x+i.bbox.w;
        int top   = i.bbox.y;
        int bot   = i.bbox.y+i.bbox.h;
	std::cout<< "Class: " << i.bbox.obj_id << "\n";
        std::cout<< "Coords: (" << left << "," << right << "," << top << "," << bot << ")\n";
	//std::cout<< "Coords (initial): (" << i.bbox.x << "," << i.bbox.y << "," << i.bbox.w << "," << i.bbox.h << ")\n";
